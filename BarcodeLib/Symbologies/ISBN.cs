    ...
    class ISBN : BarcodeCommon, IBarcode
    {
        ...
        private string Encode_ISBN_Bookland()
        {
            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data))
                Error("EBOOKLANDISBN-1: 只能是数字");//Error("EBOOKLANDISBN-1: Numeric Data Only");
            ...
            if (type == "UNKNOWN") Error("EBOOKLANDISBN-2: 无效输入. 必须送978开始，长度为9，10，12，13个字符."); //Error("EBOOKLANDISBN-2: Invalid input.  Must start with 978 and be length must be 9, 10, 12, 13 characters.");
