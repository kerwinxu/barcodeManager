...
namespace BarcodeLib
{
    ...
    public class Barcode: IDisposable
    {
        ...
        public Image Encode(TYPE iType, string StringToEncode, int Width, int Height, float fDPIX, float fDPIY)
        {
            this.DPIX = fDPIX;
            this.DPIY = fDPIY;
        ...
        internal Image Encode()
        {
            ...
            if (Raw_Data.Trim() == "")
            {
                throw new Exception("数据数据不允许为空白");
                //如下为原先的
            ...
            {
                throw new Exception("没有指定条形码编码类型");
                //如下是原先的
            ...
            switch (this.Encoded_Type)
            {
                ...
                case TYPE.FIM:
                    ...
                    break;
                default: throw new Exception("不被支持的条形码编码类型");
                    // 如下是原先的
        ...
        private Bitmap Generate_Image()
        {
            ...
            if (Encoded_Value == "") throw new Exception("首先必须被编码");
            Bitmap b = null;
                ...
                case TYPE.ITF14:
                    {
                        ...
                        //如下是我添加的
                        b.SetResolution(this.DPIX, this.DPIY);
                        ...
                        if (iBarWidth <= 0 || iquietzone <= 0)
                            throw new Exception("指定的图像大小不够大，无法绘制图像");
                ...
                using (Graphics g = Graphics.FromImage(img))
                {
                    ...
                    switch (LabelPosition)
                    {
                        case LabelPositions.BOTTOMCENTER:
                            ...
                            LabelY = img.Height - (int)(font.Height*this.DPIY/96);
                            f.Alignment = StringAlignment.Center;
                            break;
                    ...
                    g.FillRectangle(new SolidBrush(this.BackColor), new RectangleF((float)0, (float)LabelY, (float)img.Width, (float)(font.Height * this.DPIY / 96)));
                    ...
                    //draw datastring under the barcode image
                    g.DrawString(this.RawData, font, new SolidBrush(this.ForeColor), new RectangleF((float)0, (float)LabelY, (float)img.Width, (float)(font.Height * this.DPIY / 96)), f);
