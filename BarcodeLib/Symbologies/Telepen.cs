...
namespace BarcodeLib.Symbologies
{
    ...
    class Telepen: BarcodeCommon, IBarcode
    {
        ...
        private void EncodeASCII(string input, ref string output)
        {
            ...
            {
                Error("ETELEPEN-1: 无效的数据当编码ASCII时"); //Error("ETELEPEN-1: Invalid data when encoding ASCII");
            }//catch
        }
        private void EncodeNumeric(string input, ref string output)
        {
            ...
            {
                if ((input.Length % 2) > 0)
                    Error("ETELEPEN-3: 试图奇数个字符编码"); //Error("ETELEPEN-3: Numeric encoding attempted on odd number of characters");
            ...
            {
                Error("数字编码失败");//Error("ETELEPEN-2: Numeric encoding failed");
            }//catch
        }
