...
namespace com.google.zxing.qrcode.decoder
{
	...
	sealed class DecodedBitStreamParser
	{
		...
		private static System.String guessEncoding(sbyte[] bytes)
		{
            ...
            //因为这个在解码后会尝试判断编码，如果她判断错误，后边不管设置是什么编码，都会乱码，
            //所以我要自己判断一下是否是中文的，这个方法没有判断中文的

            ClsGuessEncoding guess = new ClsGuessEncoding();//这个类是判断编码的
            string strEncoding = guess.GetEncodingString(bytes);
            if (strEncoding != "OTHER")//如果不是其他编码，就返回。
                return strEncoding;
