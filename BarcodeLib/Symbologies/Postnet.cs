    ...
    class Postnet : BarcodeCommon, IBarcode
    {
        ...
        private string Encode_Postnet()
        {
            ...
            switch (Raw_Data.Length)
            {
                ...
                case 9:
                case 11: break;
                default: Error("EPOSTNET-2: 无效的数据长度. (只能是 5, 6, 9, or 11 数字)"); //Error("EPOSTNET-2: Invalid data length. (5, 6, 9, or 11 digits only)"); 
                    break;
            }//switch
            ...
            foreach (char c in Raw_Data)
            {
                ...
                {
                    Error("EPOSTNET-2: 无效数据. (只能是数字) --> " + ex.Message);//Error("EPOSTNET-2: Invalid data. (Numeric only) --> " + ex.Message);
                }//catch
            }//foreach
