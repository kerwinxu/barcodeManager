    ...
    class Code128 : BarcodeCommon, IBarcode
    {
        ...
        private void BreakUpDataForEncoding()
        {
            ...
            if (this.type == TYPES.C)
            {
                if (!IsNumeric(Raw_Data))
                    Error("EC128-6: C128-C 中只有数字可以被编码.");
        ...
        private void InsertStartandCodeCharacters()
        {
            ...
            if (this.type != TYPES.DYNAMIC)
            {
                switch (this.type)
                {
                    ...
                    case TYPES.C: _FormattedData.Insert(0, "START_C");
                        break;
                    default: Error("EC128-4: 未知的固定编码启动类型."); //Error("EC128-4: Unknown start type in fixed type encoding.");
                        break;
                }
            }//if
            else
            {
                                ...
                                while (error)
                                {
                                    ...
                                    { 
                                        ...
                                        if (col++ > CurrentCodeSet.ItemArray.Length)
                                            Error("没有开始字符."); //Error("No start character found in CurrentCodeSet.");
                                    }//catch
                                }//while
                ...
                {
                    Error("EC128-3: 开始和代码无法插入字符.\n 消息 : " + ex.Message); //Error("EC128-3: Could not insert start and code characters.\n Message: " + ex.Message);
                }//catch
            }//else
        }
        private string GetEncoding()
        {
            ...
            foreach (string s in _FormattedData)
            {
                ...
                if (E_Row == null || E_Row.Length <= 0)
                    Error("EC128-5: 无法发现编码值( " + s1 + " ) 在 C128 类型 " + this.type.ToString());
