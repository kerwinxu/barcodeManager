...
namespace BarcodeLib.Symbologies
{
    ...
    class UPCE : BarcodeCommon, IBarcode
    {
        ...
        {
            if (Raw_Data.Length != 6 && Raw_Data.Length != 8 && Raw_Data.Length != 12) Error("EUPCE-1: 无效数据长裤 (只能是 8 或 12 位数字)");// Error("EUPCE-1: Invalid data length. (8 or 12 numbers only)");

            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data)) Error("EUPCE-2: 只能是数字.");// Error("EUPCE-2: Numeric only.");
            ...
            if (Raw_Data.Length == 12)
            {
                ...
                if (NumberSystem != 0 && NumberSystem != 1)
                    Error("EUPCE-3: 无效的数字系统（只有0和1是有效的）"); //Error("EUPCE-3: Invalid Number System (only 0 & 1 are valid)");
                ...
                else if (!Manufacturer.EndsWith("0") && Int32.Parse(ProductCode) <= 9 && Int32.Parse(ProductCode) >= 5)
                {
                    ...
                    Error("EUPCE-4: 非法UPC-A输入的转换。无法转换."); //Error("EUPCE-4: Illegal UPC-A entered for conversion.  Unable to convert.");
