    ...
    class UPCA : BarcodeCommon, IBarcode
    {
        ...
        private string Encode_UPCA()
        {
            ...
            if (Raw_Data.Length != 11 && Raw_Data.Length != 12)
                Error("EUPCA-1: 无效数据长度. (长度必须是 11 或 12  位)"); //Error("EUPCA-1: Data length invalid. (Length must be 11 or 12)");

            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data))
                Error("EUPCA-2: 只能是数字"); //Error("EUPCA-2: Numeric Data Only");
            ...
            {
                Error("EUPCA-3: 未找到国家分配商代码."); //Error("EUPCA-3: Country assigning manufacturer code not found.");
            }//catch
        ...
        private void CheckDigit()
        {
            ...
            {
                Error("EUPCA-4: 错误计算校验位."); //Error("EUPCA-4: Error calculating check digit.");
            }//catch
        }//CheckDigit
