...
namespace BarcodeLib.Symbologies
{
    ...
    class UPCSupplement2 : BarcodeCommon, IBarcode
    {
        ...
        {
            if (Raw_Data.Length != 2) Error("EUPC-SUP2-1: 无效的数据长度. (只能是长度为 2)"); //Error("EUPC-SUP2-1: Invalid data length. (Length = 2 required)");

            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data))
                Error("EUPC-SUP2-2: 只能是数字"); //Error("EUPC-SUP2-2: Numeric Data Only");
            ...
            catch { Error("EUPC-SUP2-3: 无效数据. (只能是数字)"); }
