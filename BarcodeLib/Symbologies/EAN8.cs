...
namespace BarcodeLib.Symbologies
{
    ...
    class EAN8 : BarcodeCommon, IBarcode
    {
        ...
        {
            //check length
            if (Raw_Data.Length != 8 && Raw_Data.Length != 7) Error("EEAN8-1: 数字长度有问题 (只能是7和8位数字)"); //Error("EEAN8-1: Invalid data length. (7 or 8 numbers only)");

            //check numeric only
            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data)) Error("EEAN8-2: 只能是数字."); //Error("EEAN8-2: Numeric only.");
